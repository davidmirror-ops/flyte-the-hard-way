...
2. Once created, verify its configuration and take note of the IPv4 address:
```bash
...
=======
- b . Replace `127.0.0.1` with the IP address of the VM (see step 2)
- c.  Replace the `default` name with `k3s-01` or other descriptive name for the cluster.
>>>>>>> Stashed changes
...
- cluster:
  ...
  name: k3s-01
```
- d. Scroll down to the `contexts` section and add a new context. Change the `user` and `context` names from `default` to something more descriptive:
...
- context:
  ...
  name: k3s-01
```
- e. Scroll down to the `users` section and add a new entry. Make sure to set a `name` that matches the `user` field in the `context` you just created:
